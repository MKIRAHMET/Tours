...
// Query to retrieve total number of announcements
...
?>
    ...
    <h1>ΑΝΑΚΟΙΝΩΣΕΙΣ</h1>
    <a href="new_an.php" style="background-color: #3498db; color: #fff; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin-bottom: 10px; display: inline-block;">ΝΕΑ ΑΝΑΚΟΙΝΩΣΗ</a>
...
$sql = "SELECT id, title, publication_date, status FROM announcements";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<p style='font-size: 20px; margin-bottom: 8px;'><strong>ΤΙΤΛΟΣ:</strong> <a href='edit_announ.php?id={$row["id"]}' style='text-decoration: none; color: #3498db; font-weight: bold;'>{$row["title"]}</a></p>";
        echo "<p style='margin-bottom: 5px;'><strong>ΗΜΕΡΟΜΗΝΙΑ:</strong> {$row["publication_date"]} <span style='margin-left: 20px;'><strong>ΚΑΤΑΣΤΑΣΗ:</strong> {$row["status"]}</span></p>";
        echo "</div>";                      
        echo "<p><a href='edit_announ.php?id={$row["id"]}' style='background-color: #3498db; color: #fff; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin-bottom: 10px; display: inline-block;'>ΕΠΕΞΕΡΓΑΣΙΑ</a></p>";
        echo "<p><a href='delete_announ.php?id={$row["id"]}' style='background-color: #e74c3c; color: #fff; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block;'>ΔΙΑΓΡΑΦΗ</a></p>";
        echo "<hr>";   
    }
} else {
